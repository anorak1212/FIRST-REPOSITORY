...
def metodo_posicion_falsa(func, a, b, tol, cifras):
    if func.subs(x, a)*func.subs(x, b) >= 0:
        print("No se puede aplicar el método de la posición falsa en el intervalo dado.")
        return
    c = a
    iteracion = 0
    ...
    while True:
        c_anterior = c
        c = b - ((func.subs(x, b)*(b - a)) / (func.subs(x, b) - func.subs(x, a)))
...
print("La raíz es: ", round(metodo_posicion_falsa(f, a, b, tol, cifras), cifras))
